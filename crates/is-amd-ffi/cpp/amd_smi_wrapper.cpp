...
int64_t collect_temperature(amdsmi_processor_handle h) {
    // Try multiple temperature sensors in order of preference:
    // 1. Junction/Hotspot (most relevant for GPU thermal throttling)
    // 2. Edge (GPU die edge temperature)
    // 3. VRAM (video memory temperature)
    int64_t temp = 0;

    // Try junction/hotspot first
    if (amdsmi_get_temp_metric(h, AMDSMI_TEMPERATURE_TYPE_JUNCTION,
                                AMDSMI_TEMP_CURRENT, &temp) == AMDSMI_STATUS_SUCCESS) {
        if (temp > 0) {
            // AMD SMI may return millidegrees or degrees depending on driver version
            return (temp > 1000) ? (temp / 1000) : temp;
        }
    }

    // Try edge temperature
    ...
    // Try VRAM temperature
    if (amdsmi_get_temp_metric(h, AMDSMI_TEMPERATURE_TYPE_VRAM,
                                AMDSMI_TEMP_CURRENT, &temp) == AMDSMI_STATUS_SUCCESS) {
        if (temp > 0) {
            return (temp > 1000) ? (temp / 1000) : temp;
        }
